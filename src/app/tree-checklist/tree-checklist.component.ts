...
//to take value of item selected
  ...
  GetSelected(node: TodoItemFlatNode): void {
    if(this.checklistSelection.isSelected(node)){
      this.arr.push(node.item);
      ...
      var index = this.arr.indexOf(node.item);
      if (index > -1) {
  ...
  checkRootNodeSelection(node: TodoItemFlatNode): void {
    ...
    console.log(node.item+"-checkRoot");
    const descendants = this.treeControl.getDescendants(node);
    
    const descAllSelected = descendants.every(child =>
      this.checklistSelection.isSelected(child)
    );
    console.log(nodeSelected+"-checkRoot");
    console.log(descAllSelected+"-descALl");
  ...
  getParentNode(node: TodoItemFlatNode): TodoItemFlatNode | null {
    ...
    for (let i = startIndex; i >= 0; i--) {
      ...
      if (this.getLevel(currentNode) < currentLevel) {
        console.log(currentNode.item+"-currentNode"); //parent node
        console.log(currentNode.level+"-currentLevel"); // level of parent node
        return currentNode;
      }
    }
    return null;
